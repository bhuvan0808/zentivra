...
class DigestCompiler:
    ...
    async def compile(
        ...
        Args:
            run_trigger_id: Internal FK of the RunTrigger.
            findings: List of finding dicts from agents.

        Returns:
            dict with executive_summary, sections, total_findings, date, etc.
        """
        logger.info(
            "digest_compile_start run_trigger_id=%s findings=%d",
            run_trigger_id, len(findings),
        )
        ...
        logger.info(
            ...
            dedup_result.total_unique, dedup_result.total_duplicates,
        )
        ...
        try:
            ...
            executive_summary = self._fallback_executive_summary(ranked_findings, sections)
        ...
        logger.info(
            ...
            len(sections), len(ranked_findings),
        )
        return digest_data
    ...
    def _fallback_executive_summary(self, findings: list[dict], sections: dict) -> str:
        total = len(findings)
        parts = [f"Today's AI Radar detected **{total} findings** across the AI landscape.\n"]
