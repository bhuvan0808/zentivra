"""Async Valkey client for session management.

Falls back to DB-only token validation when Valkey is unreachable.
Uses the redis.asyncio library (wire-compatible with Valkey).
"""
...
class ValkeyClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.warning("valkey_unavailable error=%s — falling back to DB-only auth", exc)
            self._redis = None
            self._available = False
    ...
    async def get_session(self, auth_token: str) -> dict | None:
        """Return parsed session dict or None."""
        if not self._available or not self._redis:
            return None
        try:
    ...
    async def delete_sessions(self, auth_tokens: list[str]) -> None:
        """Remove multiple session keys from Valkey (bulk invalidation)."""
        ...
        try:
            ...
            logger.warning("valkey_bulk_delete_failed count=%d error=%s", len(auth_tokens), exc)
